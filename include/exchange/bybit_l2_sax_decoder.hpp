...
public:
    BybitL2Sax(BybitL2Message& message, std::vector<trading::PriceLevel>& bids,
               ...
               std::int64_t quantity_scale)
        : message_(message), bids_(bids), asks_(asks), price_scale_(price_scale), quantity_scale_(quantity_scale) {}
...
private:
    ...
    BybitL2Message& message_; std::vector<trading::PriceLevel>& bids_; std::vector<trading::PriceLevel>& asks_; std::int64_t price_scale_, quantity_scale_; Key key_{Key::Other}; Side side_{Side::None}; int depth_{}; bool expect_price_{true}; trading::Price price_{};
};
} // namespace detail
...
inline bool decode_bybit_l2(std::string_view payload, std::int64_t price_scale,
    ...
    detail::BybitL2Sax sax{message, bids, asks, price_scale, quantity_scale};
