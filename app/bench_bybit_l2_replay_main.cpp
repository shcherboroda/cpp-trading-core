...
int main(int argc, char** argv) {
    if (argc < 2) return 1;
    const bool scanner = argc > 2 && std::string(argv[2]) == "scanner"; const bool bounded = argc > 2 && std::string(argv[2]) == "bounded"; const bool verify_bounded = argc > 2 && std::string(argv[2]) == "verify-bounded"; const bool copy_topic = argc > 2 && std::string(argv[2]) == "copy-topic";
    ...
    if (verify_bounded) {
        ...
        for (std::size_t i = 0; i < frames.size(); ++i) {
            exchange::BybitL2Message sax_message, bounded_message;
            std::vector<trading::PriceLevel> sax_bids, sax_asks, bounded_bids, bounded_asks;
            ...
            if (sax_ok != bounded_ok || sax_message.type != bounded_message.type || sax_message.topic_matches != bounded_message.topic_matches || sax_message.ts != bounded_message.ts || sax_message.cts != bounded_message.cts || !same_levels(sax_bids, bounded_bids) || !same_levels(sax_asks, bounded_asks)) {
    ...
    for (std::size_t repeat = 0; repeat < repeats; ++repeat) {
        ...
        for (const auto& frame : frames) {
            ...
            if (scanner) { if (!exchange::decode_bybit_l2_scanner(frame, bids, asks)) continue; message.type = frame.find("\"type\":\"snapshot\"") != std::string::npos ? "snapshot" : "delta"; } else if (bounded) { if (!exchange::decode_bybit_l2_bounded(frame, 10, 1'000'000, message, bids, asks, "orderbook.50.BTCUSDT") || !message.topic_matches) continue; } else if (!exchange::decode_bybit_l2(frame, 10, 1'000'000, message, bids, asks, "orderbook.50.BTCUSDT", copy_topic, &diagnostics) || !message.topic_matches) continue;
    ...
    std::cout << (scanner ? "scanner" : bounded ? "bounded" : copy_topic ? "copy-topic" : "direct-topic") << ',' << total.size() << ',' << percentile(decode,.5) << ',' << percentile(decode,.99) << ',' << percentile(level_arrays,.5) << ',' << percentile(level_arrays,.99) << ',' << percentile(apply,.5) << ',' << percentile(apply,.99) << ',' << percentile(total,.5) << ',' << percentile(total,.99) << '\n';
}
