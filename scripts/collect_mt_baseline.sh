...

{
  ...
  echo "git_status:"
  git status --short
  echo "command=taskset -c $CPUS $BIN $EVENTS $SEED --latency=$LATENCY_MODE --format=csv"
  echo "cpus=$CPUS"
  uname -a
  echo
...
echo "run,processed,seed,queue_capacity,warmup_events,latency_mode,elapsed_ns,throughput_events_per_s,mean_ns_per_event,latency_samples,p50_ns,p95_ns,p99_ns" > "$OUTPUT_DIR/runs.csv"

for run in $(seq 1 "$RUNS"); do
  result="$(taskset -c "$CPUS" "$BIN" "$EVENTS" "$SEED" "--latency=$LATENCY_MODE" --format=csv)"
