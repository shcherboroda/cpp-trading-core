...
mkdir -p "$OUTPUT_DIR/raw"
{
  ...
  echo "source_sha256=$(sha256sum "$ROOT_DIR/app/bench_market_data_book_main.cpp" "$ROOT_DIR/src/market_data_order_book.cpp" "$ROOT_DIR/scripts/collect_l2_depth.sh")"
  echo "git_status:"
  git status --short
  echo "command=taskset -c $CPU $BIN --levels-per-side=$LEVELS_PER_SIDE --iterations=$ITERATIONS --warmup=$WARMUP --mode=$MODE --format=csv"
  uname -a
  g++ --version | head -n 1
...
echo "run,mode,levels_per_side,iterations,warmup,updates_per_batch,cpu_start,cpu_end,elapsed_ns,batches_per_second,p50_ns,p95_ns,p99_ns,checksum" > "$OUTPUT_DIR/runs.csv"
for run in $(seq 1 "$RUNS"); do
  result="$(taskset -c "$CPU" "$BIN" "--levels-per-side=$LEVELS_PER_SIDE" "--iterations=$ITERATIONS" "--warmup=$WARMUP" "--mode=$MODE" --format=csv)"
